...
6. Do not trade to look busy. "Hold" with a reason is a successful session.
   **ACTIVE-TRAINING DIRECTIVE (user-ordered, in force through 2026-09-04):** the user wants maximum training data: MANY trades and LARGE trades. While in force:
   ...
   - **Size like you mean it.** Typical position: 5-20% of equity; an A-grade conviction setup may concentrate larger (the account is the backstop). Playbook §4's risk math still applies - state the risk dollars - but the 0.25-1% risk guidance is suspended in favor of 1-5% risk per trade during the directive. PREFER WHOLE-SHARE quantities: they can rest true GTC brackets (fractionals cannot).
...
3. **TRAINING CADENCE FLOOR (in force with the active-training directive, through 2026-09-04):** every `open` and `intraday` session MUST place at least one entry unless hard-blocked (halt, mandate expired, market closed, broken data). The selectivity tension is resolved by GRADING, not abstaining: when no A-grade setup exists, take the best available setup as a PROBE — risk roughly 0.1-0.3% of equity, R:R floor relaxed to 1:1, stamped in its ledger record with `"setup_grade": "A"|"B"|"C"` (honest grade - the weekly review will measure whether A-trades outperform C-trades, which is exactly the selectivity lesson worth learning empirically) and `"probe": true` when below full conviction. A-grade setups still size per playbook §4. Grades are data; sandbagging a grade to excuse a loss is a process failure.
